...
TEST(pell_test, pell) {
    auto calc = [](int D, int N) {
        return sorted(pell<int64_t>(D, N, factor_integer_slow(abs(N))));
    };

    // x^2 - 13 y^2 = +1
    EXPECT_EQ((vector<quadraticX<int64_t>>{{ 649, 180 }}), calc(13, +1));
    // x^2 - 13 y^2 = -1
    EXPECT_EQ((vector<quadraticX<int64_t>>{{ 18, 5 }}), calc(13, -1));
    // x^2 - 3 y^2 = +1
    EXPECT_EQ((vector<quadraticX<int64_t>>{{ 2, 1 }}), calc(3, +1));
    // x^2 - 3 y^2 = -1
    EXPECT_EQ((vector<quadraticX<int64_t>>{}), calc(3, -1));
    // x^2 - 23 y^2 = +1
    EXPECT_EQ((vector<quadraticX<int64_t>>{{ 24, 5 }}), calc(23, +1));
    // x^2 - 23 y^2 = -1
    EXPECT_EQ((vector<quadraticX<int64_t>>{}), calc(23, -1));

    // x^2 - 157 y^2 = +12
    EXPECT_EQ((vector<quadraticX<int64_t>>{{ 13, 1 }, { 10663, 851 }, { 579160, 46222 }, { 483790960, 38610722 }, { 26277068347, 2097138361 }, { 21950079635497, 1751807067011 }}), calc(157, +12));
    // x^2 - 157 y^2 = -12
    EXPECT_EQ((vector<quadraticX<int64_t>>{{ 50, 4 }, { 2719, 217 }, { 2271269, 181267 }, { 123363799, 9845503 }, { 103049745749, 8224265053 }, { 5597138921710, 446700316396 }}), calc(157, -12));

    // x^2 - 13 y^2 = 108
    EXPECT_EQ((vector<quadraticX<int64_t>>{ {-15, 3}, {-11, 1}, { 11, 1 }, { 15, 3 }, { 24, 6 }, { 41, 11 }, { 80, 22 }, { 141, 39 }, { 249, 69 }, { 440, 122 }, { 869, 241 }, { 1536, 426 }}), calc(13, 108));
}
...
TEST(pell_test, pell_solutions) {
    ...
    // x^2 - 13 y^2 = 108
    auto vs10 = sorted(pell<int64_t>(13, 108, factor_integer_slow(108), 0, 0, 10));
    EXPECT_EQ(to_quadx(13, { { 11, 1 }, { 15, 3 }, { 24, 6 }, { 41, 11 }, { 80, 22 }, { 141, 39 }, { 249, 69 }, { 440, 122 }, { 869, 241 }, { 1536, 426 } }), vs10);

    auto vs10K = sorted(pell<int64_t>(13, 108, factor_integer_slow(108), 10000, 0, 0));
    EXPECT_EQ(to_quadx(13, { { 11, 1 },  { 15, 3 }, { 24, 6 }, { 41, 11 }, { 80, 22 }, { 141, 39 }, { 249, 69 }, { 440, 122 }, { 869, 241 }, { 1536, 426 }, { 2715, 753 }, { 4799, 1331 }, { 9479, 2629 } }), vs10K);

    auto vs50K = sorted(pell<int64_t>(13, 108, factor_integer_slow(108), 0, 50000, 0));
    EXPECT_EQ(to_quadx(13, { { 11, 1 }, { 15, 3 }, { 24, 6 }, { 41, 11 }, { 80, 22 }, { 141, 39 }, { 249, 69 }, { 440, 122 }, { 869, 241 }, { 1536, 426 }, { 2715, 753 }, { 4799, 1331 }, { 9479, 2629 }, { 16755, 4647 }, { 29616, 8214 }, { 52349, 14519 }, { 103400, 28678 } }), vs50K);
}
