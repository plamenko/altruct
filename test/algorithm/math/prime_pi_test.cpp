...
TEST(prime_pi_test, prime_pi) {
	...
	for (int k = 0; k < pi2.size(); k++) {
		...
		EXPECT_EQ(pi2[k], prime_pi(powT(2LL, k), prim.pi().data(), prim.p().data())) << "2^" << k;
	}
	vector<ll> pi10 = { 0, 4, 25, 168, 1229, 9592, 78498, 664579 };
	for (int k = 0; k < pi10.size(); k++) {
		prime_pi_PHI_tbl<ll, int>().clear();
		EXPECT_EQ(pi10[k], prime_pi(powT(10LL, k), prim.pi().data(), prim.p().data())) << "10^" << k;
	}
	for (int m = 0; m < 1000; m++) {
		EXPECT_EQ(prim.pi(m), prime_pi(m, prim.pi().data(), prim.p().data())) << "pi(" << m << ")";
	}
}
