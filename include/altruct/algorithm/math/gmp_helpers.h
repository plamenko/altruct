...
#define ALTRUCT_USE_MPF_CLASS
//#define ALTRUCT_USE_MPFR_CLASS
...
template<> mpz altruct::math::castT<mpz, mpf>::of(const mpf& x) {
    mpz res;
    mpz_set_f(res.get_mpz_t(), x.get_mpf_t());
    return res;
};
...
inline mpf f_ceil(mpf x) { mpf r; mpf_ceil(r.get_mpf_t(), x.get_mpf_t()); return r; }
