...
public:
	...
	series(const T& c0 = T(0)) : my_series_members(1), p(c0) { p.reserve(N()); }
	...
	series(int c0) : my_series_members(1) , p(c0) { p.reserve(N()); } // to allow constructing from 0 and 1
	series(series&& rhs) : my_series_members(rhs.N()), p(std::move(rhs.p)) { p.reserve(N()); }
	series(const series& rhs) : my_series_members(rhs.N()), p(rhs.p) { p.reserve(N()); }
	series(polynom<T>&& rhs, int _N) : my_series_members(_N), p(std::move(rhs)) { p.reserve(N()); }
	series(const polynom<T>& rhs, int _N) : my_series_members(_N), p(rhs) { p.reserve(N()); }
	series(polynom<T>&& rhs) : my_series_members(rhs.size()), p(std::move(rhs)) { p.reserve(N()); }
	series(const polynom<T>& rhs) : my_series_members(rhs.size()), p(rhs) { p.reserve(N()); }
	series(std::vector<T>&& c) : my_series_members((int)c.size()), p(std::move(c)) { p.reserve(N()); }
	series(const std::vector<T>& c) : my_series_members((int)c.size()), p(c) { p.reserve(N()); }
	template<typename It> series(It begin, It end) : my_series_members((int)std::distance(begin, end)), p(begin, end) { p.reserve(N()); }
	series(std::initializer_list<T> list) : my_series_members((int)list.size()), p(list) { p.reserve(N()); }

	series& swap(series& rhs) { p.swap(rhs.p); std::swap(refN(), rhs.refN()); return *this; }

	series& resize(int _N) { refN() = _N; p.resize(_N); return *this; }
	...
	series  operator -  (const series &rhs) const { series t(*this); t -= rhs; return t; }
	series  operator -  ()                  const { return series(-p, N()); }
	series  operator *  (const series& rhs) const { series t(*this); t *= rhs; return t; }
	series  operator /  (const series& rhs) const { series t(*this); t /= rhs; return t; }
	...
	series derivative() const { return series(p.derivative(), N()); }
	series integral() const { return integral(p.ZERO_COEFF); }
	series integral(const T& c0) const { auto pi = p.integral(c0); pi.resize(N()); return series(std::move(pi), N()); }

	// t(x) so that s(x) * t(x) == 1 + O(x^N); O(M(N))
	series inverse() const {
		// ensure that p[0] is 1 before inverting
		if (p[0] == zeroT<T>::of(p[0])) return series(polynom<T>{ p.ZERO_COEFF }, N());
